...
class TestApiRouter(TestCase):
    def test_path_args(self):
        resp = self.client.get(
            '/intro/get_request_with_path_args', dict(arg1='abc', arg2='def')
        )
        self.assertEqual(resp.status_code, 404)

        resp_json = self.client.get('/intro/get_request_with_path_args/abc/def').json()
        self.assertEqual(resp_json['arg1'], 'abc')
        self.assertEqual(resp_json['arg2'], 'def')

    def test_query_args(self):
        resp_json = self.client.get(
            '/intro/get_request_with_query_args', dict(arg1='abc', arg2='def')
        ).json()
        self.assertEqual(resp_json['arg1'], 'abc')
        self.assertEqual(resp_json['arg2'], 'def')
